...
int main(int argc, char *argv[])
{
    ...
    while (alarmCount < maxRetransmissions && !sucess)
    {
        if (alarmEnabled) {
        ...
        if (ua > 0) {
            ...
            if (result == 1) {
                if (isUAFrame(byte)) {
                    ...
                    alarm(0);           
                    alarmEnabled = 0;
                    printf("UA received. Alarm disabled.\n");
                }
            }
        }
        if (!alarmEnabled && !sucess) {
            if (alarmCount < maxRetransmissions) {
                printf("Resending SET frame...\n");
                writeBytesSerialPort(set_frame, set_frame_size);
                alarmEnabled = 1;
                alarm(3);
            }
            else {
        ...
        {
            STOP = TRUE;
            printf("Received 5 char. Stop reading from serial port.\n");
        }
    }
